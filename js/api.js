...
export async function scanCompel(API_BASE,brands,{signal,onMessage}={}){
  const r=await fetch(`${API_BASE}/api/scan`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({brands}),signal});
...
const _jsonTry=t=>{let j=null;try{j=JSON.parse(t)}catch{j=null}return j};
export async function dailyGet(API_BASE,{date,password,want}={}){
  const r=await fetch(`${API_BASE}/api/daily/get`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({date,password,want})});
  const t=await r.text().catch(()=>""),j=_jsonTry(t);
...
export async function dailySave(API_BASE,{kind,adminPassword,readPassword,data}={}){
  const r=await fetch(`${API_BASE}/api/daily/save`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({kind,adminPassword,readPassword,data})});
  const t=await r.text().catch(()=>""),j=_jsonTry(t);
