...
export function parseDelimited(text){
  ...
  const split=line=>{const out=[];let cur='',q=false;for(let i=0;i<line.length;i++){const ch=line[i];
    ...
    else if(!q&&ch===delim){out.push(cur);cur=''} else cur+=ch
  } out.push(cur);return out.map(v=>v.trim())};
  let hdr=null,rows=[];
  for(const line of lines){if(!line||!line.trim())continue;if(!hdr){hdr=split(line);continue}
...
export function stockToNumber(raw,{source}={}){
  ...
  let t=s;
  if(t.includes('.')&&t.includes(','))t=t.replace(/\./g,'').replace(/,/g,'.'); else t=t.replace(/,/g,'.');
  t=t.replace(/[^0-9.\-]/g,'');
